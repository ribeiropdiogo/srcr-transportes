...
bfsr(Y,[n(S,P1)|Ns],C,'spublicidade',P) :- findall(n(S1,[[S,S1,A]|P1]),
        (percurso(S,S1,A,T), nao(publicidade(S)),
        nao(publicidade(S1)),\+ member(n(S1,_),C)), Es),
        append(Ns,Es,O),
		    append(C,Es,C1),
        bfsr(Y,O,C1,'spublicidade',P).

bfsr(Y,[n(S,P1)|Ns],C,'nabrigado',P) :- findall(n(S1,[[S,S1,A]|P1]),
        (percurso(S,S1,A,T), nao(abrigado(S)),
        nao(abrigado(S1)),\+ member(n(S1,_),C)), Es),
        append(Ns,Es,O),
        append(C,Es,C1),
        bfsr(Y,O,C1,'nabrigado',P).
